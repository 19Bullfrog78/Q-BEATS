...
- **Stato:** 🔴 **OPEN — 🚨 BLOCCANTE PALCO** (non si può avere il Follower che prosegue dopo lo stop). Parzialmente coperto da Bug 4 (`6d1dbbf`) per il ciclo background/foreground; questo scenario è **foreground attivo prolungato**, distinto. Round dedicato **dopo** il merge Bug 2.b. **Servono log iPad/rete** (nel log Follower la causa non c'è: zero righe wifid/AWDL/mDNS attorno all'evento).
- **Dominio:** CC (+ rete).
...
- **🟢 CHIUSO 11/06/2026 — entrambe le ferite, device-validate + mergiato su master `ee0cbc0` (squash).**
