...
### TD #17 — Link perde peer dopo sessioni lunghe (🚨 BLOCCANTE PALCO — CONFERMATO 11/06, caratterizzato 16/06)
...
- **Stato:** 🔴 **OPEN — 🚨 BLOCCANTE PALCO** (non si può avere il Follower che prosegue dopo lo stop). Parzialmente coperto da Bug 4 (`6d1dbbf`) per il ciclo background/foreground; questo scenario è **foreground attivo prolungato**, distinto.
- **NEXT STEP reale** (sostituisce il vecchio «cattura log in sessione lunga foreground», **già eseguito** 15-18/06): inchiodare il **differenziatore degli 8 roam**. **Strategia 1** = ri-analisi dei log ESISTENTI (vicinanza al cambio-sezione + lunghezza del buco RX `delta_in` + RSSI/BSSID per ogni roam) → candidato + spec di cosa il banco deve riprodurre. **Strategia 2** = banco con **ROAMING reale (2 AP stesso SSID)** + dual-capture. ⚠️ Il **VR2800 single-AP NON riproduce il roam** (che è proprio il grilletto). **Niente fix Layer 2 sui soli log.**
- **Dominio:** CC (+ rete).
