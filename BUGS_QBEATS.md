...
- **🔄 RICLASSIFICAZIONE 10/06/2026 (TEST5+TEST6, ratificata Mauro+referee) — DUE FERITE DISTINTE (radici diverse, prove intra-giro). Dettaglio: memoria CC `project_qbeats_bugs_tracker.md` entry 10/06.**
  - **FERITA A — nascita/ingresso** (= il "9-di-8 a barra piena", bloccante-palco originale; = §TD-follower-rejoin **faccia (A) "corsa di fase all'aggancio"**): il Follower entra sfasato e resta sfasato, **flusso click reale PULITO**. Causa: il callback Link avvia `start()` PRIMA del setup del runner (Opzione C, `AudioEngine.swift:520-544`) → l'attesa d'ingresso è calcolata col **quantum stantio** del giro precedente (TEST5/6 RUN1/6: attesa=5−fase su 1ª sez 4/4) e/o sulla **timeline pre-azzeramento** del Director (RUN2/7).
    - **STATO: 🟢 CHIUSA 10/06/2026 — device-validata TEST6 + ratificata Mauro+referee.**
    - **FIX-A — commit `7904ca8`** (branch `fix/bug-2b-ferita-a-join-al-fire` da `4e144c8`; α `63fc3d2` esclusa = non-antenato; pushato solo-branch; **SOLO L3** = solo `AudioEngine.swift`, +218/−153, zero C++; CI run `27275998351` verde, *Verify entitlements* ✓ = TD#44 intatto). Nuovo metodo `armSharedJoin(retriesLeft:)`: **ingresso FINALIZZATO AL FIRE** — ogni arm ri-legge fase/quantum/tempo correnti; al fire ri-valida il target e **ri-arma** (bounded `kSharedJoinMaxRearms=2`) se non cade su confine di barra; corpo d'ingresso (join/seed/pre-roll/SPY) **trapiantato verbatim** (contenuto parola-per-parola identico, solo re-indentato per la migrazione in metodo). È **esattamente** la "Direzione fix proposta" di §TD-follower-rejoin ("ri-legge la fase Link AL FIRE / attende il beat-zero del Director").
    - **VALIDAZIONE DEVICE — TEST6 10/06** (IPA run `27275998351` "472", 7 giri + 2 check orecchio, dati `LOG\RUN\TEST 6 472`): **zero fallback d'ingresso** (nessuna riga `seed SKIP su SHARED`) su 7/7 — vs 3/8 in TEST5 — con `[LINK][SHARED] downbeat raggiunto … startBeat:0.0427` su 7/7; **`re-arm` SOLO nei giri col metro lasciato stantio** (RUN5/7 = fix in azione; RUN6 = pass-through legittimo); **`linkPhase_fire=0.0465` identico 7/7** = atterraggi sul battere (asse Link). **Partenze giudicate pulite a orecchio (Mauro) + lato-marker coerente** (re-arm solo 2/7, downbeat 7/7); confronto "7/7 ok vs 4/8 rotte TEST5" = orecchio+marker. `WARN aggancio` 7/7 = **artefatto di misura PREESISTENTE** (asse MIDI `midi_engine_get_beat_at_time`, identico in TEST5 anche nei giri puliti), NON ingresso storto.
    - **Regressione START LOCAL "2 di 4" ESCLUSA** (ricerca storica read-only): documentata dal **29/05** (Issue B microbar fantasma "osservato in START LOCAL" `ff530b7`; BOX3 V69 R2; Faccia D `b128536` 01/06) + il blocco SEED è trapiantato verbatim → stesso codice di `4e144c8` e precedenti.
    - **NOTA GATE:** chiusa solo dopo device (TEST6 ✓ Mauro+forensic) **E** ratifica (Mauro+referee 10/06) — entrambe soddisfatte. master `bbdd06a` intatto, **merge HELD** finché anche Ferita B non è chiusa.
  - **FERITA B — click mangiati/ri-sparati a runtime** (= il "Bug 2.b" originale qui sotto, accento audio errato): ogni correzione `sync_phase` che scavalca un mark **annichilisce o duplica un click reale**. Vive in **`setBeatPositionTimeOnly`** (`MetronomeDSP.cpp:208-230`, `ceil` amnesico `:215-218`) — *NB riconciliazione: la citazione storica `setBeatPosition cpp:166-180` è pre-Parte-1; post fix Parte 1 il ramo runtime usa `setBeatPositionTimeOnly`, `setBeatPosition` è solo-Resume*. ε=½ campione; soglia `0.01` < drift `0.025` → ~2000 correzioni/giro. Keystone: TEST5-RUN3 tripla (3 colpevoli ↔ 3 buchi WAV 1.002s ↔ 3 skip spia) + RUN2 doppio-click; **TEST6-RUN3 in flagrante** (2 risparî backward sui mark **1771/1779** = doppio accento + accento sul beat 2 udìti). Riconciliazione **14/14 sezioni** (`netto mangiati−risparati = span−spec`). Lettura-thread = **same-thread** (`processBuffer`/`setBeatPositionTimeOnly` un solo callsite ciascuno, stessa `scheduleNextBuffer`, solo audioQueue) → **TD-beat-drop UNIFICATO qui; fix-B = SOLO numerico** (mark-preserving + soglia), niente lock. **🔴 OPEN — prossimo round FIX-B (L1+L2, audit RT-thread, diff SEPARATO, mai impacchettato col fix-A).**
...
- **Causale: IDENTIFICATA (31/05, read-perno cross-device + dato device Run 2).** `MetronomeDSP::setBeatPosition` (`core_engine/MetronomeDSP.cpp:166-180`) ri-derivava `_currentBeatInBar = ceil(beatPosition - _startAbsoluteBeat) % _beatsPerBar`, con `_startAbsoluteBeat` = origine del Play **non ri-ancorata al confine di sezione**. Alimentata dal **beat assoluto** scritto da `sync_phase` (`link_engine_sync_phase` → `*outNewBeatPosition = linkBeat`) via `metronome_set_beat_position` nel ramo Follower (`AudioEngine.swift:2117`). Al cambio BPB la fase sfasa (es. confine a 32 beat, `32 % 3 = 2` → accento sul beat 2). Standalone pulito perché non chiama `setBeatPosition`; Director immune perché usa `assert_session_state` (scrive verso l'esterno, non ri-deriva il DSP locale). **Seme `_sectionBeatCounter` scagionato** dal dato device (sfasamento fisso, identico a prescindere dall'ingresso).
- **Direzione fix: 2 — separare allineamento temporale (`sync_phase`) da fase di battuta (`_currentBeatInBar`).** La fase resta governata dal contatore di sezione (avanzamento DSP `cpp:267`, ancorato al downbeat dallo swap SEAMLESS). **Direzione 1 scartata** (ri-ancorare `_startAbsoluteBeat` al confine): insegue il sintomo, riapre il rischio sul Resume, accumula su più cambi di sezione consecutivi.
- **Forma (Parte 1 — committata):** nuova primitiva DSP **solo-temporale** `MetronomeDSP::setBeatPositionTimeOnly` (identica a `setBeatPosition` meno il blocco `beatIdx`/`beatInBar`/`_currentBeatInBar`) + bridge `metronome_set_beat_position_time_only`; il ramo `sync_phase` instrada il metronomo sulla variante time-only (`AudioEngine.swift:2117`), `midi_engine_set_beat_position` e ramo Director **invariati**. `setBeatPosition` **intatta per il solo Resume**. Commit **`45faa90`** sul branch `fix/bug-2b-visual-section-sync` (NON pushato, NON mergiato).
- **Faccia D — offset di BARRA con aggancio PULITO (dato duro, pesa sul merge):** run 01/06 ore 15:08 su IPA **#466** (`98f7a0f`). **Conferma `git log`:** `45faa90` è antenato di `98f7a0f` → #466 **include il fix Parte 1**. Tutte le partenze con `startBeat:0.0427`, frazioni piccole (0.09–1.29) = **aggancio pulito**. Ciononostante iPad mostra "**bar 2 di 4**" e iPhone "**bar 3 di 4**" → **il fix Parte 1 NON elimina l'offset di barra a partenza pulita**: il counter di barra è un **asse separato** dalla corsa di fase (faccia A in `TD-follower-rejoin`). **Indizio, NON conclusione:** `[SinkDiag-AQ] beatTick:32` a **due soli campioni** (15:08:46, 15:09:50) — possibile counter fermo, ma 2 righe non bastano; 32 = confine prima sezione BPB Mixed (8×4), stessa aritmetica di Bug 2.b. **Discriminante (serie completa `beatTick`) da RI-CATTURARE** (log 15:08 perso). **Implicazione merge:** il counter offset "bar N di M" **sopravvive** al fix Parte 1 → la chiusura di Bug 2.b sul fronte counter NON è data dal solo fix Parte 1.
- **Stato:** 🔴 **OPEN ALTA — 🚨 BLOCCANTE PALCO** (Fase 6-7-bis). **Fix Parte 1 committato (`45faa90`), in attesa test device — NON chiuso** (Layer 1, regola `feedback_qbeats_chiuso_solo_dopo_device`). Validazione richiesta: re-test Run 2 (4/4→3/4 con peer = accento sul beat 1) + multi-sezione (4/4→3/4→4/4→6/8) + nessuna regressione Director/standalone. Severità decisa 31/05 (Mauro + referee): accento audio errato in Collaborative ai cambi di **time-signature puri** (BPM costante) = **errore musicale, non cosmesi**; il costrutto che lo innesca (es. strofa 4/4 → bridge 3/4 stesso tempo) è **musica normale, non edge case** → soglia di prodotto, non del palco specifico. Elevato da 🟠 e ricollocato da Sez. 1.2 a Sez. 1.1.
- **Follow-up (Parte 2 — esclusa dal commit `45faa90`):** esposizione **latente** del Resume in peer — `setBeatPosition` (callsite `AudioEngine.swift:670`) condivide la radice ma è raggiungibile dal Follower/peer solo via recovery di sistema **non gated-Link** (route-change/engine-config/app-wakeup/pending-recovery `:2508`/`:2804`/`:2837`/`:2856`/`:2868` passano `recoveryBeat` non-nil; l'interruption `:2606` invece passa nil sotto Link). Hardening proposto (solo Layer 3): passare `resumeAtBeat = nil` sotto Link anche in quei recovery, allineandoli alla convenzione `:2606`. **Subordinato a conferma device** che l'esposizione sia reale; oggi NON device-confermato.
...
- La **quarta faccia (D — offset di barra a partenza pulita)** è registrata sotto **Bug 2.b**.
...
- **Stato:** **faccia (A) = Ferita A → 🟢 CHIUSA 10/06/2026** (device-validata TEST6 + ratificata Mauro+referee, fix `7904ca8`). **Facce (B) gap-reset L3→L1 [= UI segmenti stantii], (C) runner-non-resetta, (D) offset-barra: 🔴 RESTANO OPEN.** Causale (B/C/D) = ipotesi forte (non identificata). **NON chiuse** (regola `feedback_qbeats_chiuso_solo_dopo_device`).
- **Verifiche residue — log 15:08–15:09 NON più disponibile (né device né chat) → da RI-CATTURARE in un test futuro; il FIX resta gated su questi dati:**
  ...
  - serie completa `[SinkDiag-AQ]` / `beatTick` (faccia D in Bug 2.b: contatore fermo o no — finora solo 2 campioni a 32, insufficienti);
  - righe `beatsPerBar` / sezione corrente (facce B/D).
