...
| `2026-08-30_QLive-Player_IL-VELO-DICE-DA-DOVE__END-SHOW-sullo-scaffale-e-sei-decisioni-incise__390x844.html` | **NORMATIVO sul player fermo (bivio, velo) e sul DETTAGLIO — sei decisioni incise, CD 30/08/2026.** Abolisce il selettore delle stanze nel dettaglio dello show attivo (D④): **sul dettaglio il contratto 18/07 resta senza oggetto**, per dichiarazione di CD, non per abrogazione. ⛔ **NON tocca la LISTA:** le due porte del contratto 18/07 lì restano vive e invariate. Vedi la marcatura sulla riga del contratto 18/07 sopra. ⛔ **Superato sui punti 3, 4 e 5 dal foglio `IL-FOLLOWER-NON-TOCCA-IL-TRASPORTO` dell'11/09** (count-in alla ripresa: vale LIBRO · il velo non porta count-in finche' non suona · scaffale 41). |
| `2026-09-11_..._IL-FOLLOWER-NON-TOCCA-IL-TRASPORTO__...__2300x8730.html` | **NORMATIVO per il perimetro del Follower** — le lastre del suo player, velo, dettaglio, muto/mixer, e la schermata a collegamento caduto; piu' la lista dopo Remove e la copy EN del chooser. ⛔ **SUPERATO sul punto 12** (la regola di leggibilita') e sulle correzioni 5.1-5.4 dai due fogli successivi di questa stessa data. ⚠️ **La sua regola sull'iPad e' superata** dalla decisione di Mauro dell'11/09: l'iPad si fa a parte, non scalando l'iPhone (LIBRO, riga 2026-09-11). |
| `2026-09-11_..._LA-SCALA-DI-PALCO__...__1860x4350.html` | ⛔ **SUPERATO su tabella, lastre e correzioni 5.1-5.4** dal foglio `LA-TABELLA-FINALE` di pari data. Il resto regge. ⚠️ **Contiene una prescrizione di codice SBAGLIATA, ritirata da CD stesso nel foglio successivo:** dice che il Follower dovrebbe ignorare gli start/stop in arrivo. Seguirla farebbe continuare a suonare i Follower dopo lo STOP del Direttore. **Non seguirla.** Il difetto vero e' il verso opposto — `TD-follower-stop-propaga`. |
| `2026-09-11_..._LA-TABELLA-FINALE__P17-e-le-maiuscole-a-21__1860x3440.html` | **NORMATIVO per la scala di palco.** La sua tabella e' incisa in BOX5 V48 («SCALA DI PALCO — P = 17, SOLO iPHONE»): se un giorno divergono, vince BOX5. ⚠️ **Contiene un'affermazione FALSA:** che la data dello show stia dentro `Setlist.name` «per modello dati», citando `CLAUDE.md §4`. `CLAUDE.md` non e' un canonico, e `Setlist` ha un campo `date: Date` separato (`ios_app/QBeats/Models/Setlist.swift:5-6`). Le tre uscite che quel foglio propone per il nome in testata **poggiano su quella premessa**: si riaprono quando si ridisegnera' il player. |
| `2026-09-11_PROVA-30cm__STAGE-SECONDARY__390x844.html` | **STRUMENTO DI MISURA, non un mockup dell'app.** Prima prova di Mauro su iPhone 13 (11/09). Esito: a 13 pt si legge ma non e' comodo. Da qui nasce il criterio del comfort. |
| `2026-09-11_SCHEDA-GRADINI__SCALA-DI-PALCO__390x844.html` | **STRUMENTO DI MISURA.** Seconda prova di Mauro (11/09): da qui vengono P = 17 e il 21 delle maiuscole spaziate. ⚠️ **ERRATA dichiarata da CD:** nel blocco ⑤ «GRADINO 15 — corpo 18» va letto **19**, e la riga `Bridge · 121 · 6/8` sta al pavimento invece che al livello DATA. **I quattro numeri di Mauro non ne sono toccati:** vengono dai blocchi ①-④, scale pure di un tipo alla volta. |
| `2026-09-11_MISURA_impronte-sha256__foglio-11-09-e-prova-30cm.html` | **NOTA DI MISURA, seconda emissione.** Porta peso e impronta presi prima del trasporto della mattina dell'11/09. Non e' un foglio di disegno e non governa nulla. |
