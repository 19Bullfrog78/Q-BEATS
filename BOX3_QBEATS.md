BOX3 V98 — 2026-07-22 (AUTOPORTANTE) · HEAD=origin=9784294 (⚠️ campo strutturalmente stale by-design, come Sez.5 di LIBRO: cita l'HEAD al momento della scrittura, NON aggiornato dopo; chi ha bisogno dell'HEAD vero verifica `git log -1` a fonte — verificato a fonte 22/07: HEAD = origin/master = `9784294c05437dc727a9618014d147e0d9fab66e` = LIBRO v39; CI `29855951057` verde, `headSha` coincidente con HEAD, unica run per quel commit)
...
Mirror di consegna di QUESTO BOX3 V98 — **REGIME NUOVO**, ratificato 21/07 (LIBRO v39, Sez.2 riga
2026-07-21; spec in BOX5 V27). **BOX3 NON è più untracked:** `BOX3_QBEATS.md` vive in root del repo,
è TRACCIATO, ha `-text` in `.gitattributes` (disco = blob al byte) e si modifica IN PLACE — niente
più un file nuovo per versione, le versioni precedenti stanno nella storia git.
Propagazione a E: = una **STAMPA** col nome per-versione ancorato al commit **INTRODUTTIVO** (forma
`BOX3_V98_2026-07-22_<commit>.md`, in `FILE X CLAUDE.MD\BOX3_Codice\`), verificabile contro il blob
a quel commit. Stessa regola per la copia caricata nel Progetto Claude: nome per-versione, **MAI
nome fisso** (V98 (f)).
⚠️ SUPERSEDE il blocco di coda di V97, falso su due punti: si autodescriveva «V95» e dichiarava
«BOX3 è untracked», istruendo il regime sbagliato.

**STATO DI QUESTO FILE:** V98 canonica corrente. Il read-back sha256 dei target si dichiara nel
