...
**Versione:** V52 — 2026-09-16
**Decisione:** 2026-09-16 — ⚠️ **il campo porta il giorno in cui la decisione è stata presa, NON quello del deposito** (R-δ.15, ratificata Mauro 06/09/2026). La data del deposito non si scrive: vive in git. ⇒ **V52 — il perimetro del Follower (mandati A360→A362, diff ratificati dal referee): il Follower non manda a Link né stop (`AudioEngine.stopSync`) né avvio (`AudioEngine.start`, decisione `FollowerStartDecision`: entra solo in una sessione che suona, altrimenti resta fermo); una regola sola per il ruolo (`FollowerDecision`: ruolo E Link acceso dall'utente); fascia del Follower = riga-regola ed EMERG; usciti `WaitingForDirectorView`, START LOCAL, CANCEL e `.waitingForDirector`; slot E del velo; riga ambra assente sul Follower; pedale fermo sul Follower (muto escluso); R-δ.22, i numeri dei mandati li assegna CC al cancello. Marcature su «Transport», «Overlay Standby», «Stati Vista LIVE», «Invarianti tecnici Layer 3», «MODELLO DI SESSIONE Q-LIVE» §4.**
...
📌 **MARCATURA 16/09/2026 (A360/A362) — DUE FASCE, UNA REGOLA. Le due righe qui sopra restano come furono scritte: si marca.** A chi comanda il trasporto (Direttore, Solo) i tasti reali sono **sei** — sezione indietro, play/stop, sezione avanti, loop, KILL BASE, emerg (`TransportView`, `commandStrip`), invariati. Al **Follower** la fascia della lastra ④ del foglio CD 11/09 `IL-FOLLOWER-NON-TOCCA-IL-TRASPORTO`: la riga-regola «TRANSPORT · DIRECTOR ONLY» (STAGE-CAPS, a capo come nella lastra, a sinistra su due celle) e il solo EMERG, nella sua cella di oggi (`TransportView`, `followerStrip`); muto (testata) e maniglia del mixer restano; la fascia conserva la sua altezza (la decide `LiveView`). Ratifica: LIBRO `2026-09-11` «CRITERIO GENERALE DEL PERIMETRO DEL FOLLOWER» (tolti PLAY, STOP, tocco sul velo, KILL BASE; restano muto, mixer, EMERG). Chi è Follower lo dice `FollowerDecision` («Invarianti tecnici Layer 3», riga «Chi comanda il trasporto», marcatura V52); `audioEngine` resta `let` (riga «`let` vs `@ObservedObject` su View figli»): la forma della fascia si legge nel `body`, e a player aperto il ruolo non cambia.
...
⚠️ **MARCATURA 15/09/2026 (mandato A355) — LE QUATTRO RIGHE QUI SOPRA DESCRIVONO IL VELO DI PRIMA. Non si riscrivono: si marca qui com'è ADESSO, con le fonti.** Sedi: `StandbyOverlayView` (a schermo), `LiveView` (decisione e tocco), `LiveHeaderView` (testata), tipo puro `StandbyOverlayDecision` in `ios_app/QBeats/Models/` (testato nel banco `QBeatsTests`), token `QLiveStage` accanto a `QLiveTheme` in `QLiveKit.swift`.

- **Tre righe, in tutti i casi, e nessuna riga di count-in** (invariante «Il velo NON porta righe di count-in finché il count-in non suona», V48). **Riga A, relazione:** «Resume from ⟨sezione⟩» se c'è un punto di ripresa (indice di sezione > 0) **e** il nome della sezione, tolti gli spazi, non è vuoto; altrimenti «Next:» — garanzia contro la bugia (foglio CD 11/09 `IL-FOLLOWER-NON-TOCCA-IL-TRASPORTO`, lastra ①). **Riga B:** il nome della canzone. **Riga C, gesto:** «Tap anywhere» a chi comanda il trasporto, «The director starts» al Follower (lastra ⑥). **La riga A va a capo fino a due righe, centrata, a capo sulle parole; ellissi solo oltre la seconda** — il nome della sezione è identificazione come quello della canzone (LA-TABELLA-FINALE §③: una riga in più è meglio di un nome tagliato). **La riga C resta su una riga:** le sue due stringhe sono fisse. Fonti: LIBRO riga `2026-08-30` «⑤ IL VELO DEL PLAYER DEVE DIRE DOVE RIPARTE», LIBRO riga `2026-09-09/11` «IL TRASPORTO È DEL DIRETTORE», LIBRO riga `2026-09-11` «CRITERIO GENERALE DEL PERIMETRO DEL FOLLOWER» (il tocco sul velo è tolto al Follower).
- **Un solo dato per il testo e per il tocco.** Le tre righe e il ramo della ripartenza escono dallo stesso tipo puro: indice e nome della sezione corrente dal runner (lo stesso oggetto che `startCurrentSection` fa suonare), nome della canzone dal caso `.standby(nextSongName:)` — che **non si rinomina**: «la canzone che parte al prossimo tocco» è vero in tutti i casi — e ruolo calcolato dal chiamante con la regola incisa `audioEngine.currentLinkMode == .collaborativa` (riga «Chi comanda il trasporto», V48), **senza `linkEnabled`**, che l'app spegne da sola in background a click fermo. Banco: sezione 0 · > 0 con nome · nome vuoto e di soli spazi · Follower con e senza ripresa · gigante a 12/13/25/26 caratteri.
- **Token della scala di palco, in un posto solo** (`QLiveStage`): righe A e C **STAGE-CAPS** (21, JetBrains Mono 600, spaziatura 1,5, maiuscole, bianco 0,60); riga B **STAGE-HERO** (68, Inter 900, spaziatura −1,6, bianco pieno) con **STAGE-NEXT** (44) come pavimento del gigante — fino a 12 caratteri una riga che scende fino a 44, da 13 a 25 due righe a 44, oltre 25 ellissi; distanze A→B 14 e B→C 34 (11 e 26 della lastra ① × k = 1,308); margine orizzontale 26 (LA-TABELLA-FINALE §③, larghezza utile 338). Fra apparecchi `token × max(1, scaleFactor)` sui corpi; distanze e margine in punti, come il resto della Vista LIVE — legge provvisoria, l'iPad si fa a parte (LIBRO `2026-09-11` «L'iPAD SI FA A PARTE, DOPO L'iPHONE»). ⚠️ La riga «Font: Inter-Black, 52pt * scaleFactor» qui sopra è **superata**: 52 non è nella scala. ⚠️ L'interlinea 1,04 del foglio (`.vnm`) non e' costruita: a schermo vale quella predefinita del carattere. Si guarda al collaudo.
- **Il nome si scrive come è scritto:** cade il maiuscolo forzato, che il foglio (`.vnm`) non ha. La pulsazione di 2,2 s resta (riga «Pulse animation» qui sopra: vale). Il `GeometryReader` e lo spazio in alto 0,27 restano (riga «GeometryReader interno preservato»: vale): il velo parte sotto la testata, ma lo spazio in alto è 0,27 dell'altezza intera (velo + testata) meno la testata — la posizione verticale è quella di prima.
- **Il velo è corto** (decisione 13, capitolo «MODELLO DI SESSIONE Q-LIVE» §1): si ferma sotto la testata; freccia e muto restano pieni e toccabili; il centro della testata e i LED scendono al 10% col resto. Chi comanda il trasporto: il tocco vale sotto la testata, e il ramo lo decide il punto di ripresa, senza seconda condizione. **Follower: il tocco sul velo non fa partire niente** (LIBRO `2026-09-11` «CRITERIO GENERALE DEL PERIMETRO DEL FOLLOWER»); il velo continua a coprire i tasti al 10% sotto di sé; la sua uscita è la freccia (LIBRO `2026-09-09/11` «PERIMETRO DEL FOLLOWER A SHOW ATTIVO»). `leavePlayer()` non è toccato.
- Strumentazione passiva `[Q-BEATS][A355]`: una riga a ogni comparsa del velo (ruolo, caso, indice di sezione, nome vuoto sì/no), una a ogni tocco (ruolo e ramo preso; sul Follower «nessuna azione»).

**Decisioni del referee in questo mandato, ognuna col motivo:**
- **(a) Il velo del Follower nomina la sezione da cui riparte il SUO apparecchio**, letta dallo stesso dato della sua ripartenza (`QLiveSession.orchestrateDirectorPlay`, ramo standby: `runner.currentSectionIdx` → `startCurrentSection`). Se al collaudo non coincide con quella del Direttore, è un difetto di sincronia da riparare alla radice, non un motivo per togliere il nome. Chiude il punto aperto Ⓞ del foglio CD 11/09 («Il velo del Follower può nominare la sezione?»).
- **(b) Il velo corto si costruisce ora:** tolto il tocco al Follower, la freccia è la sua unica mossa (LIBRO `2026-09-09/11` «PERIMETRO DEL FOLLOWER A SHOW ATTIVO»).
- **(c) Il nome sul velo si scrive come è scritto**, senza maiuscolo forzato.
- **(d) iPad:** la legge provvisoria del resto della Vista LIVE, `token × max(1, scaleFactor)`, finché l'iPad non si fa a parte (LIBRO `2026-09-11` «L'iPAD SI FA A PARTE, DOPO L'iPHONE»).

⚠️ **CONSEGUENZA CHE CAMBIA UNA PREMESSA — rivalutazione a Mauro; BUGS non si tocca in questo mandato.** Da questo commit un apparecchio col ruolo Follower **non parte dal velo neanche a Link spento** (il ruolo è `currentLinkMode`, non `linkEnabled`), e in attesa il PLAY del transport è disabilitato (`TransportView`, `disabled: isStandby`). La gravità di `TD-mode-picker-senza-solo` (🟠 OPEN MEDIA / NON BLOCCANTE PALCO, ratificata Mauro 22/08/2026) poggiava su «si può sempre suonare da soli spegnendo Link»: **in Q-Live non vale più** — chi ha scelto Follower una volta, e dal selettore non torna a Solo, non parte da solo nemmeno col Link spento.

📌 **MARCATURA 16/09/2026 (A360/A362) — IL RUOLO DEL VELO E LO SLOT E. Il blocco A355 qui sopra resta come fu scritto: si marca.** «Ruolo calcolato dal chiamante con la regola incisa `audioEngine.currentLinkMode == .collaborativa` … senza `linkEnabled`» è **superato nel meccanismo e rispettato nel motivo**: il ruolo viene da `FollowerDecision` (`Models/`) — ruolo **E** Link acceso dall'utente (`ABLLinkIsEnabled`, mirror `AudioEngine.linkUserEnabled`, scritto solo dal callback di `link_engine_set_is_enabled_callback`, cioè dal pannello Link) — e l'interruttore dell'app, quello che `QBeatsApp` muove in background a click fermo, entra nella regola per essere ignorato: al rientro un Follower legge ancora «The director starts». `LiveView` costruisce `StandbyOverlayDecision` da `audioEngine.followerDecision` (testo, tocco e slot E da un solo dato). **Slot E** (lastra ⑧ del foglio CD 11/09, `.vlink`): Follower senza nessun apparecchio collegato (`linkIsConnected` falso) → sotto la riga C due righe, «No device connected» in STAGE-CAPS ambra `#f5b820` (valore già in uso: muto, lampo di KILL BASE) e «nothing will start from here» in **STAGE-SECONDARY**, entrato in `QLiveStage` con i valori della tabella P = 17 (17 · JetBrains Mono 500 · spaziatura 0,3 · bianco 0,60, minuscola com'è scritta); C→E 26 × k = 34, fra le due righe 5 × k = 7; nessun tocco, nessuna scorciatoia, nessun invito a spegnere Link. Dato: `StandbyOverlayDecision.showsNoDeviceLines` (= Follower E nessuno collegato: a chi comanda lo slot non compare mai, la garanzia sta nel dato). ⚠️ Eredita `TD-link-indicator-stale`: spia dei collegati indietro ⇒ «No device connected» col collegamento in piedi.

📌 **MARCATURA 16/09/2026 (A360/A362) — LA PREMESSA TORNA VERA.** Con `FollowerDecision` un ruolo Follower con Link spento dal pannello è Solo in tutto: fascia intera, «Tap anywhere», il tocco fa partire, bivio e RESUME disponibili, il motore parte come Solo (a Link spento `link_engine_probe_session` rende tutto a zero e `start()` prende il ramo standalone). «Si può sempre suonare da soli spegnendo Link» vale di nuovo in Q-Live; la gravità di `TD-mode-picker-senza-solo` si rivaluta in BUGS nel giro del collaudo. ⚠️ L'interruttore Link di `DebugView` non è quello dell'utente (muove `link_engine_set_enabled`, non `ABLLinkIsEnabled`): un Follower acceso da lì si comporta da Solo pur attivo su Link — l'IPA di palco è compilato Debug.
...
📌 **MARCATURA 16/09/2026 (A360/A362) — GLI STATI TORNANO OTTO.** `.waitingForDirector` è uscito da `Models/LivePlaybackState.swift` insieme a `WaitingForDirectorView`, START LOCAL e CANCEL (LIBRO `2026-09-10` «NIENTE START LOCAL»; ticket `TD-waiting-for-director-start-local-da-togliere`, tolta e non lasciata irraggiungibile): il Follower non ha PLAY, nessuno scrive più quell'attesa; il Follower a collegamento caduto aspetta sul velo, informato («The director starts» + slot E), o chiude con END SHOW dal dettaglio. Restano otto `case`, `.starting` compreso; la lista di permessi di `SetlistRunner.primeDisplay` non cambia. L'enumerazione del LIBRO (Sez.1 «Stati LiveSession») si aggiorna nel giro del collaudo.
...
| Link — fresh play branching | Modello a 4 rami: (peers==0 && !isPlaying) → standalone puro; (_linkMode == .direttore, qualsiasi stato peer) → direttore-standalone, detta la timeline con start_at_beat_zero (Director comanda sempre — i peer si adeguano); (collaborativa con peer in play) → join_running_session; resume → start_at_beat. Build #309 esteso 28/05/2026. Nota: in .direttore, start_at_beat_zero è chiamato anche con peers > 0. Comportamento progettato e ratificato. ⚠️ **MARCATURA 16/09/2026 (A361/A362) — IL FOLLOWER È DECISO IN TESTA A `start()`, PRIMA DEI RAMI** (`FollowerStartDecision`, `Models/`, banco `FollowerStartDecisionTests`): Follower (ruolo E Link acceso dall'utente) e sessione che suona (`link_engine_probe_session`) → `join_running_session`, il ramo condiviso di sempre, col beat di ripresa ignorato; Follower e sessione ferma → il motore **non parte** (pubblica `.stopped`, abbassa il gate anti double-emit), qualunque sia l'innesco — callback, ripresa dopo interruzione, cambio di configurazione, reset dei servizi media, risveglio, recupero pendente, pedale, debug. Da un Follower non escono mai `start_at_beat_zero` né `start_at_beat`. ⛔ «Almeno un collegato» (`link_engine_num_peers`) **non decide**: è la spia dei collegati, che può restare indietro (`TD-link-indicator-stale`); alla punta `3d5544d1` un Follower con la spia a zero e la sessione che suona entrava comunque nel ramo condiviso. I quattro rami qui sopra restano quelli di chi comanda il trasporto (Direttore, Solo), riga per riga. |
...
| **Chi comanda il trasporto = il predicato del PLAY** | **V48** — Decisione referee (A348, 3.3): la regola è `audioEngine.currentLinkMode == .collaborativa` ⇒ Follower (`TransportView.swift:59` @ `84e21ee1`). Il badge «FOLLOWER» di `LiveView` aggiunge la guardia `linkEnabled` (`LiveView.swift:124-128`): **è veste, non regola**. Ratifiche di Mauro 09-11/09: il trasporto è del Direttore (LIBRO v82, Sez.2). ⚠️ **MARCATURA 16/09/2026 (A360/A362) — SUPERATA:** la regola è `FollowerDecision` (`Models/`, banco `FollowerDecisionTests`): Follower = ruolo `.collaborativa` **E** Link acceso dall'utente (`ABLLinkIsEnabled`, mirror `AudioEngine.linkUserEnabled`); l'interruttore dell'app, mosso in background da `QBeatsApp`, entra nella regola per essere ignorato. La legge `AudioEngine.followerDecision` in `TransportView` (forma della fascia), `LiveView` (velo, parola del badge — la visibilità resta `linkEnabled`), `QLiveRootView` (`leavePlayer`, `onResume`), `QLiveShowDetailView` (`showsResume`, riga ambra), `executeMIDIAction`; in forma nuda su copie di coda in `stopSync` e in `start`. |
...
| **Il Follower non manda a Link né stop né avvio** | **V52** — Ratifiche LIBRO `2026-09-09/11` «IL TRASPORTO È DEL DIRETTORE», `2026-09-11` «CRITERIO GENERALE DEL PERIMETRO DEL FOLLOWER», `2026-09-10` «NIENTE START LOCAL». **Stop:** `AudioEngine.stopSync`, subito prima di `link_engine_stop` — la sola riga da cui uno stop esce dall'apparecchio; ogni `stop()` e `handleStop()` (tasto, callback, fine canzone, fine scaletta, END SHOW, uscita dalla stanza, pedale, debug) ci passa, così una porta aggiunta domani non la riapre (A360). **Avvio:** `AudioEngine.start(resumeAtBeat:)`, in testa, prima dei rami (A361/A362, riga «Link — fresh play branching»). Il Follower riceve ancora start e stop del Direttore (callback di `link_engine_set_start_stop_callback` in `init`) e si ferma o parte con lui; il Direttore ignora i peer (stesso callback, guardia `_linkMode == .direttore`). Pedale sul Follower: solo il muto (`executeMIDIAction`). ⚠️ **Restano le scritture di TEMPO del Follower verso Link:** `link_engine_set_bpm` da `setBPM` (chiamato da `SetlistRunner.prepareAndStartCurrentSection` a ogni avvio di sezione orchestrato) e `link_engine_set_bpm_and_beat_at_time` dal beat callback di `scheduleNextBuffer` (al tick dopo lo swap SEAMLESS, senza guardia di ruolo) — ticket da aprire in BUGS nel giro del collaudo (referto A361, mappa B). |
...
⛔ **MARCATURA 30/08/2026 (A290) — D9 CAMBIA SU UNA GAMBA SOLA, RATIFICATA DA MAURO. Zero parole riscritte sopra: si marca.** La riga di D9 resta agli atti come fu ratificata il 23/08. **Velocità e metrica NON si prendono più «dalla PRIMA SEZIONE della canzone», ma DALLA SEZIONE CHE IL CONTO INTRODUCE.** ✅ **All'inizio della canzone la sezione introdotta È la prima** ⇒ comportamento invariato, e **i due esempi già incisi sopra restano veri parola per parola**. ⚠️ **A metà show la sezione introdotta è quella da cui si riparte**, e lì la regola vecchia sbaglierebbe velocità e numero di click. **Esempi di Mauro, verbatim:** sezione che suonerà a 90 BPM in 3/4 → count-in **3-2-1** a 90; sezione che suonerà a 130 BPM in 4/4 → count-in **4-3-2-1** a 130. ⛔ **Le altre due gambe di D9 sono INTATTE:** i tre valori (**Off · 1 battuta · 2 battute**) e la chiusura esatta sul battito 1 della musica. 🚨 **LA SEDE NON È UNICA — misurato il 30/08, e la «sede unica» di r.920 vale per l'ETICHETTA, non per il CONTENUTO.** La stessa regola vive, senza l'etichetta `D9`, in `BOX5_QBEATS.md:573` (invariante Layer 3, marcato nello stesso giro) e in `LIBRO_MASTRO_QBEATS.md:366` (sotto l'intervallo «D8-D10», marcato nello stesso giro). 🚨 **SONDA CIECA ALL'ETICHETTA — è il difetto che ha tenuto nascoste le altre due sedi per giorni, e si scrive qui perché qui ci arriva chi cerca.** La regola era **irreperibile per ETICHETTA e reperibile solo per CONTENUTO**: `grep 'D9' LIBRO_MASTRO_QBEATS.md` rende **ZERO** — e lo zero è vero, non cecità, perché sullo stesso file `D1` rende **13** righe e `D8` ne rende **2**. Su `BOX5:573` l'etichetta non compare affatto. ⇒ La sonda che le smaschera cerca il **contenuto**: `grep -n 'PRIMA SEZIONE' *.md`. È la polarità **P2** della tassonomia — fatto che sembra vuoto. ⛔ **REGOLA CHE NE ESCE: prima di cambiare una decisione ETICHETTATA, cercarla anche per CONTENUTO — l'etichetta prova dove la decisione è NATA, non dove RISIEDE.** 🚨 **NON COSTRUIBILE OGGI, misurato a HEAD `da7deb03c491f71a684207aad10f842837c3738a`:** `startCountIn(for section: String?)` (`ios_app/QBeats/AudioEngine.swift:1617-1619`) **scarta il parametro `section`** e chiama `start()` — il corpo intero è una riga. ⇒ La sezione non arriva al conto nemmeno oggi, e D9 nuova e D9 vecchia sono **ugualmente non onorate**.
...
| **P4 — LA DIFFERENZA CHE NON C'È** | «diversi» | i due oggetti sono **identici**: la sonda ha confrontato **anche ciò che non doveva** |
| **P5 — LA FORMA PERSA CHE ERA IL DATO** | i valori **giusti** | la sonda ha **tolto la parte che decide**: i numeri tornano, la struttura no |

⚠️ **P4 e P5 sono state aggiunte il 07/09/2026 (mandato A328), e NON stanno dentro le prime tre: le tre incise rendono tutte o «zero» o un numero plausibile — P4 rende un VERDETTO («diversi») e P5 rende i valori GIUSTI.** Si identificano **per polarità, non per numero**, secondo la regola già incisa in `LIBRO_MASTRO_QBEATS.md:352`: una polarità nuova non si forza dentro un ordinale esistente. 🚨 **P5 è la più pericolosa delle cinque, ed è quella che per prima non era stata elencata:** le altre quattro rendono un numero sbagliato, P5 rende **numeri giusti in una forma mutilata**, quindi non insospettisce. ⇒ **Distinzione verbatim, rilievo di CC (A326): «Un falso zero sembra niente e ti rende prudente; una forma storpiata sembra un dato e ti rende sicuro.»**
...
| **PATTERN DEGENERE** | P3 | `grep -c $'\r'` su un file **senza CR** rende **il numero totale delle righe**, non zero: il pattern matcha ogni riga. ⚠️ Anche la forma ancorata `$'\r$'` mente allo stesso modo  ⛔ **MARCATURA 07/09/2026 (mandato A328) — LO STESSO COMANDO PRODUCE ENTRAMBE LE POLARITÀ. Zero parole riscritte sopra: si marca.** Questa voce è incisa **P3** sulla base di `LIBRO_MASTRO_QBEATS.md:352`, e là rendeva il **totale delle righe**. Il 07/09 lo stesso comando si è presentato con la polarità **OPPOSTA**: ha reso **0** dove doveva rendere **221** — il congedo `HANDOFF/CONGEDO_CC_2026-09-07_A323-TRAPPOLE-DEGLI-STRUMENTI-E-TRE-PREMESSE-FALSE.md`, 221 righe e CR **0** contati sui byte — perché il pattern arrivava **rotto** e non matchava nulla, con uscita non-zero. ⇒ **L'esito «0» non era una prova**, anche se la conclusione che se ne trasse («CR 0») era per altra via **vera**. ⇒ La regola che regge non è «diffida del totale», è quella già scritta accanto: **i CR si contano SUI BYTE, mai con `grep`**, qualunque numero il filtro renda. | confrontare l'esito col **totale** delle righe: se coincide, il filtro non ha filtrato | `LIBRO_MASTRO_QBEATS.md:352` (06/08/2026, «FALSO-*POSITIVO* DA FILTRO — PRIMA DELLA SUA SERIE») |
| **PARAMETRO IGNORATO IN SILENZIO** | P3 | l'API di GitHub Actions accetta parametri **sconosciuti senza errore** e li **scarta**: `?conclusion=success` non filtra nulla e la risposta rende l'insieme intero | **quadratura**: enumerare le classi e verificare che la somma torni col totale non filtrato ⚠️ **QUALIFICA 02/09/2026 (ratificata Mauro):** la quadratura è **necessaria e non sufficiente** — **un totale che torna non prova l'insieme.** Somme uguali si ottengono anche da insiemi diversi: la quadratura smaschera il filtro che non ha filtrato, ma non garantisce né che le classi enumerate siano quelle giuste né che siano tutte. ⇒ **Chi quadra dichiara SU QUALI CLASSI ha quadrato**, o il totale che torna è solo un numero che rassicura. | **misurato qui il 26/08/2026 — prima incisione** |
...
| **ASSENZA DALLA PROPRIA VISTA** | P2 | l'osservatore **non può vedere** il supporto dove l'oggetto vive, e legge il proprio zero come inesistenza. Non è una sonda mal scritta: è una sonda **giusta puntata dove l'oggetto non è**. Tre occorrenze: `git log --grep` su ID **mai committati** (rende zero per costruzione, ed è stato usato come controllo positivo) · `git grep` sugli **untracked** · un mount che espone «Il mio Drive» e **non** «Il mio computer», dove vive il riflesso  ⛔ **MARCATURA 07/09/2026 (mandato A328) — QUARTA OCCORRENZA, ed è del REFEREE. Zero parole riscritte sopra: si marca.** ⚠️ **La linea non è disco / non-disco: è TRACCIATO / UNTRACKED.** Il referee **misura** ciò che è tracciato e lo dichiara come misura, con l'ancora al commit; sull'**untracked** è cieco **per costruzione**, e da lì non può distinguere «non esiste» da «non lo vedo». **Caso del 07/09:** il §0 del mandato A325 dichiarava «A326 NON è mai stato emesso: è rumore di chat» mentre il referto `HANDOFF/MISURE_CC_2026-09-07_A326-SWIPE-FINE-SCALETTA.md` era depositato **su due dischi da tre ore** (14.176 byte, sha256 `d1c413f6a7c73c7f9da498da8cf1959a5bf39ab8a5b5bfd5c8e909c2fd61ead7`). ⇒ **Forma obbligata: «per quanto ne so», mai «non esiste»** — e CC, davanti a due mandati che si smentiscono, **si ferma** invece di scegliere quale vinca. ⚠️ **La regola di processo referee↔CC che ne discende NON vive qui:** è cross-team, la sua sede è `LIBRO_MASTRO_QBEATS.md` Sezione 2, e **la ratifica Mauro** — al 07/09 è **proposta, non ratificata**. | **dichiarare «invisibile da qui», MAI «non esiste»**, e nominare il supporto che non si è potuto interrogare. Un secondo supporto, o un secondo osservatore, prima di concludere | **§7 della Costituzione** (`FILE.MD/QBEATS_SYSTEM_PROMPT_V5_21_06_2026.md`) ne porta la METÀ SIMMETRICA («niente fonte verificabile, non entra»); questa è **la metà mancante** — vedi la nota sotto |
...
| **GRAFIA CHE LA SONDA NON CONTA** | P2 | il glob di `git branch --list` è **sensibile alle maiuscole**, e in questo progetto lo stesso ID vive in **due grafie**: i rami lo portano **minuscolo** (`build-check/a320-endshow-scratch`), mentre file, messaggi di commit e referti lo portano **maiuscolo**. Misurato 07/09/2026 (git 2.55.0): `--list "*A320*"` rende **0** · `--list "*a320*"` rende **2** · `--list -i "*A320*"` rende **2**, e il ramo esiste anche su `origin` | ⛔ **`-i` sempre**, su ogni gamba di ricerca di un ID. ⚠️ **La sonda scritta in A321 per chiudere «occupato che sembra libero» lo RIAPRIVA identico dentro la toppa stessa:** la quinta gamba fu incisa come `git branch -a --list` **senza `-i`**, e su un ID maiuscolo rende zero mentre il ramo c'è. ⇒ Una toppa va provata **sul caso che l'ha motivata**, non solo sul caso nuovo | 07/09/2026 (mandati A325/A326) — reperto di CC in apertura di sessione, sulla quinta gamba nata in A321 |
| **IMPRONTA CONTRO IMPRONTA SU FACCE DIVERSE** | P4 | `sha256` del **disco** confrontato con `sha256` del **blob** su un file sorgente rende `DIVERGE` **anche a contenuto identico**, perché confronta anche i **fine-riga**: il disco è CRLF, il blob è LF. Misurato 07/09/2026 (A327) su **tre file su tre**: `FineSetlistView.swift` CR **58** · `LiveView.swift` CR **775** · `TransportView.swift` CR **145**, e in ogni caso **righe disco = righe blob** — firma esatta di un file CRLF, un `0x0D` per riga. ⚠️ Letto alla lettera inviterebbe a **buttare la misura giusta** | ⛔ **mai** confrontare le due facce grezze. **Normalizzare i fine-riga su entrambe** prima dell'impronta; **più una via indipendente**: `git status` e `git diff` sul percorso, che rendono **0** quando l'albero è pulito. Controprova aritmetica: **`byte_disco − byte_blob` deve valere il numero di righe** | 07/09/2026 (A327) — `HANDOFF/MISURE_CC_2026-09-07_A327-APPENDICE-A326.md` §3 |
| **FORMA STORPIATA DALLO STRUMENTO** | P5 | uno strumento di presentazione **rimuove una parte del dato** mentre riporta il resto correttamente: un `awk` di rinumerazione che **azzera l'indentazione** rende tutte le righe giuste e tutti i numeri di riga giusti, e mangia gli **spazi iniziali**. Misurato 07/09/2026 (A326) su `ios_app/QBeats/UI/Live/LiveView.swift`, dove **l'indentazione ERA la risposta**: decide se `.gesture(` pende dal **padre** o dal **fratello**. 🚨 Fidandosene, la risposta al mandato sarebbe stata **SÌ invece di NO** | ⛔ leggere in **forma fedele** — `cat -n`, o `sed` senza post-trattamento — **ogni volta che la struttura È il dato**: annidamento, indentazione, ordine, allineamento, colonne. ⚠️ Il sospetto non arriva dal numero, che è giusto: arriva dal **chiedersi che cosa lo strumento abbia potuto togliere** | 07/09/2026 (A326) — `HANDOFF/MISURE_CC_2026-09-07_A326-SWIPE-FINE-SCALETTA.md` §9 |
...
- **`D9` CAMBIA SU UNA GAMBA, ratificata da Mauro il 30/08/2026** — velocità e metrica del conto alla rovescia si prendono **dalla sezione che il conto introduce**, non più «dalla prima sezione della canzone». All'inizio coincidono (comportamento invariato, esempi vecchi ancora veri); a metà show no. **Marcata, non riscritta** (r.938).
- 🚨 **La «sede unica» di D9 è FALSA per il CONTENUTO, misurato il 30/08.** La stessa regola vive anche in `BOX5_QBEATS.md:573` (invariante Layer 3) e `LIBRO_MASTRO_QBEATS.md:366`, **senza mai usare l'etichetta `D9`** — quindi invisibili a `grep 'D9'`. Entrambe marcate nello stesso giro. ⇒ **Regola che ne esce: prima di cambiare una decisione etichettata, cercarla anche per CONTENUTO.**
- ⛔ **D9 non è onorabile dal codice di oggi**: `startCountIn(for section:)` (`AudioEngine.swift:1617-1619`) scarta il parametro `section`. Vale per la D9 nuova come per la vecchia.
...
📌 **MARCATURA 15/09/2026 (mandato A355) — LA DECISIONE 13 È COSTRUITA:** il velo si ferma sotto la testata; la freccia (e il muto) restano scoperti e toccabili; il centro della testata e i LED scendono al 10% col resto. Com'è fatto, e con quali fonti: capitolo «Overlay Standby (tra canzoni)», marcatura A355. Sul Follower il tocco sotto la testata non fa partire niente (LIBRO `2026-09-11` «CRITERIO GENERALE DEL PERIMETRO DEL FOLLOWER»).
...
📌 **MARCATURA 16/09/2026 (A360/A362) — SUL FOLLOWER LA RIGA AMBRA NON COMPARE.** Dopo A360 il Follower non manda STOP a Link (`AudioEngine.stopSync`): il suo END SHOW ferma solo il proprio apparecchio, e «this will stop other devices too» direbbe il falso — la categoria che il §D del rev3 esiste per vietare. `QLiveShowDetailView`, `endShowRow`: la sottoriga compare solo a chi comanda il trasporto (`linkIsConnected` **e** non Follower, `FollowerDecision`); a chi comanda resta identica. Il bivio non la costruiva già (RETTIFICA 1 di A343). Il testo del §4 qui sopra resta come fu scritto.
...
### R-δ.11 — CANCELLO: UNA CITAZIONE NUOVA PORTA L'ANCORA, E L'ANCORA DEVE RISOLVERE (ratificata Mauro 02/09/2026)

⛔ **QUESTO CAPITOLO NON SCRIVE UNA REGOLA NUOVA.** La regola esiste dal 02/08 e non si riscrive: `LIBRO_MASTRO_QBEATS.md:344 @ c504ee4d9a0d1fd2986c64718e65a58b5ecb55d9` dispone «ogni rimando nuovo nomina il documento; se porta un numero di riga, porta `@ <commit a 40>`», ed è a sua volta estensione ai documenti della regola ratificata il 01/08 per i sorgenti, `LIBRO_MASTRO_QBEATS.md:335 @ 7ec6c1b86a7acb869c1f927fa4833374ffabb0cc`. Il 18/08 è stata registrata **viva e non onorata** (`LIBRO_MASTRO_QBEATS.md:359 @ c504ee4d9a0d1fd2986c64718e65a58b5ecb55d9`), con divieto esplicito di riscriverla. ⇒ **Qui non si ripete: le si dà il morso.**

⛔ **CANCELLO — DUE CLAUSOLE.**
**(1) PRESENZA.** Non si scrive in un canonico una citazione `FILE:riga` **nuova** priva di `@ <commit a 40>`.
**(2) RISOLUZIONE.** Non basta che l'ancora ci sia: **il commit citato deve contenere ciò che la citazione afferma**, e chi scrive lo verifica **prima** di consegnare. Un'ancora che non risolve è peggio di una citazione nuda — la nuda si vede, **l'ancora falsa sembra rigore.**
Se un mandato detta una citazione che viola l'una o l'altra, **CC si ferma e lo dichiara**, e la parte restante del mandato non parte. Vale anche quando la citazione è dettata dal referee.

🚨 **PERCHÉ LA CLAUSOLA (2) È OBBLIGATORIA: questa regola nasce dal fallimento della propria prima stesura.** La prima versione di questo capitolo, scritta il 01/09, chiedeva **soltanto** la clausola (1). **Non avrebbe intercettato il difetto trovato il 02/09**, ed è questo: il mandato incaricato di dare il morso alla regola portava una citazione **ancorata e rotta** — `LIBRO_MASTRO_QBEATS.md:344 @ 7ec6c1b86a7acb869c1f927fa4833374ffabb0cc` — copiata da un canonico e mai verificata. **Misurato dal referee il 02/09:** a quel commit la riga citata **non esiste** — la riga 344 è un separatore, e il testo citato rende **zero** sul file appiattito, con controllo positivo passato su un caso noto. Il commit `7ec6c1b8` è quello del **censimento contenuto dentro la riga**, non quello in cui la riga vive. ⇒ Una guardia che verifica la **presenza** dell'ancora invece della sua **risoluzione** controlla l'ingresso e non il risultato: è la R-δ.12, Clausola A, violata dal documento stesso che la detta.

✅ **Il fatto che regge, e il suo limite dichiarato:** la regola esiste dal 02/08; il 18/08 è stata registrata viva e non onorata; e il **primo tentativo di darle il morso** — il mandato del 02/09 — ha **propagato una citazione rotta presa da un canonico**, terza mano nota della stessa citazione. ⛔ **Quanto il difetto sia diffuso NON è misurato**, e nessuno lo dichiari misurato citando questa riga.

⚠️ **UN CANONICO NON È UNA FONTE PER LE PROPRIE CITAZIONI.** Una citazione letta in un canonico si verifica come ogni altra, e con **più** cura, non con meno: **è nei canonici che le citazioni rotte si nascondono meglio, perché nessuno le sospetta.**

⛔ **Non autorizza nessun giro di riparazione.** Le citazioni esistenti sono storia: si marcano e non si riscrivono, come già dispone la riga del 18/08 citata sopra. Il cancello vale **da qui in avanti**.

⚠️ **Perimetro:** vale sulle citazioni a **documenti** e a **sorgenti**. Non vale sui rimandi interni al documento che si sta scrivendo, che si risolvono da soli.

### R-δ.12 — LA GUARDIA SI VERIFICA SUL RISULTATO, E IL PUNTO D'INSERIMENTO SI DESCRIVE (ratificata Mauro 02/09/2026)

**Due clausole, un capitolo solo: sono lo stesso difetto visto da due lati.** Chi scrive il mandato indica una posizione con un dato che non la garantisce; chi esegue la verifica con una guardia che non la controlla. Separarle produce due regole che nessuno collega.

**CLAUSOLA A — la guardia.** ⛔ **Una guardia si verifica sul RISULTATO che produce, non sull'ingresso che riceve.**
**Perché, misurato (A304-v5, 01/09, misura di CC nel suo congedo A313 §6):** una guardia sull'ancora d'inserimento **sarebbe passata** mentre la premessa era già falsa. Il testo della riga-ancora combaciava ancora, ma non era più l'ultima riga: l'inserimento sarebbe finito nel posto sbagliato **con la guardia verde**. Verificava l'**identità** quando serviva la **posizione**.
⇒ **Una guardia verde non dice che stai facendo la cosa giusta: dice che la cosa che ha guardato è come se l'aspettava.** Prima di fidarsene si dichiara **a quale domanda risponde davvero**, non quale domanda si credeva di averle fatto.
⚠️ Identità-contro-riga, identità-contro-posizione e presenza-contro-risoluzione non sono tre regole: sono tre modi in cui fallisce questa.

**CLAUSOLA B — il punto d'inserimento.** ⛔ **Un punto d'inserimento si DESCRIVE, non si indica.**
Un numero di riga passato a chi esegue è un dato che **decade in silenzio**: resta sintatticamente valido mentre punta altrove, e non produce nessun errore. Si descrive per relazione a un contenuto stabile — «in coda al capitolo X», «immediatamente prima dell'intestazione Y» — e chi esegue verifica **che il risultato sia finito dove doveva**, non che il numero esistesse.

### R-δ.13 — DAVANTI A UN NUMERO CHE NON TORNA SI CHIEDE «COSA LO SPIEGHEREBBE» (ratificata Mauro 02/09/2026)

⛔ **REGOLA:** davanti a un numero che non torna, la domanda non è «mi impedisce di procedere?» ma **«cosa lo spiegherebbe?»**. La prima archivia, la seconda misura.

**Perché, misurato il 01/09 — due scarti che sembravano conflitti e non lo erano:** 204 contro 205 righe di Sezione 2 del LIBRO (una conta l'intestazione, l'altra no); 577 contro 578 righe dello stesso file (una conta l'elemento vuoto finale). ⚠️ L'identità del file fra le due misure fu dichiarata da chi le riconciliò e **non è stata rimisurata qui**: quel che questa riga afferma è la meccanica delle convenzioni, non l'impronta.
...
### R-δ.22 — IL NUMERO DI UN MANDATO LO ASSEGNA CC AL CANCELLO (decisione del referee 16/09/2026, incisa A362)

🚨 **Dal 16/09/2026 il numero di un mandato lo assegna CC, al cancello ID, nel momento in cui lo usa. Il referee non scrive numeri.** Il mandato arriva senza numero («lo assegna CC al cancello»); CC misura il primo ID libero sulle gambe vive di R-δ.17 — disco per nome e contenuto, indice su disco — con un controllo positivo sull'ultimo assegnato, scrive per prima la riga nell'indice (R-δ.16) e dichiara il numero nel referto e nel messaggio finale.

**Motivo, misurato:** il referee legge solo il deposito, che su `HANDOFF/` è cieco (R-δ.17): un ID preso sul disco per lui non esiste. **Tre collisioni in una settimana:** `A335` (09/09, congedo su disco riassegnato a un mandato di codice, fermato al cancello), `A354` (15/09, il mandato del velo arrivò col numero del congedo e CC prese il primo libero), `A359` (16/09, il mandato del perimetro del Follower arrivò col numero del congedo e fu fermato al cancello).

⛔ **Scartato** il commit della riga d'indice dopo ogni congedo: costerebbe un OK di Mauro e una build per riga, e reggerebbe solo finché nessuno se ne dimentica. Il cancello, invece, lo fa chi ha il disco sotto mano, ogni volta.

⚠️ Il cancello non ha mutua esclusione (`CLAUDE.md`, «Cancello ID dei mandati»): due sessioni concorrenti misurano entrambe zero. Vale R2 del LIBRO: una sola chat CC alla volta.
