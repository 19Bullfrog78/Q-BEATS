...
### ⟦S-EXIT⟧ Stato dello show appartiene allo show, non alla schermata · POST · CI+DEVICE
- **Provenienza:** scheda scritta dal referee il 01/09/2026, indirizzi verificati alla fonte sul deposito pubblico alle punte `976d986abddc027a9efdd921bc6bac634d0b30d6` e `2c9b0bf8d4e48e2d6133029245b4db8e9bf5b9e3`. Colma il buco registrato il 07/08 (Sezione C, riga datata «MARCATURA 07/08 — ⟦S-EXIT⟧ È NELL'ORDINE RATIFICATO MA NON HA SCHEDA»): ⟦S-EXIT⟧ era nell'ordine ratificato ma senza scheda.
- **Scopo:** non è «cablare l'uscita». È una frase sola, ratificata da Mauro il 22/08: **lo stato dello show deve appartenere allo show, non alla schermata.** Motivo misurato (A172, A173, 22/08): uscendo dal player senza STOP l'orologio del motore audio prosegue e l'orologio della grafica riparte, con scarto crescente a ogni rientro.
- **Già costruito, device-validato:** la memoria dello show vive nella stanza, non nella schermata — commit A242. La grafica non ricostruisce più la propria posizione: la chiede al motore — `LiveView.swift:439`, dentro l'`.onAppear` che inizia a `:324`, scritture a `:447` e `:448`. Collaudo di Mauro del 31/08, quattro cicli su quattro: uscita col «<», cinque secondi nella lista, rientro. Sezione mostrata = sezione udita, sempre. ⇒ Il difetto dei due orologi è chiuso alla causa, non mascherato.
- **Resta da costruire, una cosa sola:** l'ascolto del comando del Direttore deve uscire dalla schermata. Oggi vive dentro il player: quando il player si smonta, l'ascolto muore con lui. Misurato: col player chiuso il Follower parte al Play del Direttore ma parte cieco — click senza sezione, nome canzone vuoto, contatore oltre il totale. Non è silenzio: sembra che funzioni. ⚠️ Trappola già incisa nel tracker: sono **due mosse**, non una — la memoria (fatta) **e** l'ascolto (da fare). Chi fa solo la prima dichiara il bug chiuso in buona fede e si sbaglia. ✅ Riduce il costo: il posto dove mettere l'ascolto **esiste già** — il runner ha il proprio cassetto per queste iscrizioni e lo usa. Non è costruzione nuova, è un trasloco.
- **Cond — decisioni di Mauro, punto (d):**
  **D1, quando l'app dimentica lo show** — ⚠️ NON RISOLTA al 01/09. Misurato: uscire dalla stanza ferma il click (`AppRootView.swift:70-76`). Se la memoria dello show sopravviva o muoia a quell'uscita non è misurato da nessuno: il codice non ha nessuna riga che la svuoti fuori da `endShow()`, il commento della classe dice che muore, il tracker dice che sopravvive, e `QLiveRootView.swift` porta una marcatura §7 che dichiara non sorgentata la tesi su cui l'intera questione poggia. ⇒ Si scioglie con una prova su device, non a tavolino (protocollo nel bullet **CANCELLO** sotto). ⚠️ Ratifica di Mauro, 01/09: *dallo show si esce solo con END SHOW.* Nessuna navigazione lo chiude. ✅ **SCIOLTA SU DEVICE IL 03/09/2026 — collaudo di Mauro, tre giri su tre, un apparecchio solo.** **La memoria dello show MUORE all'uscita dalla stanza e SOPRAVVIVE all'uscita dal player.** Segnale letto sul tasto in fondo al dettaglio: dopo il «‹» **BACK TO SHOW**; dopo CASA **START SHOW**, e il click si era fermato. ⇒ **Le due porte non sono la stessa porta, ed è la misura che mancava.** Meccanismo verificato alla fonte dal referee il 03/09 a `d61e823d431a7da3c3820c6cbb815f39f2cd2b58`: `QLiveRootView.swift:76` dichiara la sessione della stanza come `@StateObject` **della schermata**, e `AppRootView.swift` monta Q-Live dentro uno `switch` sullo schermo corrente ⇒ uscire dalla stanza **smonta la schermata e con essa la sessione**. Seconda spiegazione considerata e **falsificata**: che qualcuno chiami `endShow()` uscendo verso Home — la closure d'uscita è nuda e `leavePlayer()` chiude solo a show già finito. ⇒ **Il difetto non è che lo show muoia: è che muoia senza che l'utente lo abbia chiesto**, contro la ratifica «dallo show si esce solo con END SHOW».
  **D2, il Follower che parte col player chiuso: dove si trova?** — Il Direttore preme Play; il batterista può avere in mano la lista o il dettaglio. Misurato il 01/09: nessun documento del progetto — BOX5, LIBRO, fogli CD — dice niente su questo scenario. ⛔ DOMANDA ANCORA SENZA RISPOSTA AL 01/09. Tre strade nominate: **A** l'app lo porta dentro · **B** lo lascia dov'è · **C** lo lascia dov'è ma la lista si accende e un tocco lo porta dentro (la spia «Playing» è già disegnata da CD nel foglio del 30/08). Parere del referee: **C** se è uscito lui, **A** se il comando arriva dal Direttore — l'app sposta solo quando l'evento arriva da fuori. Prezzo dichiarato della A: un Play premuto per sbaglio in soundcheck fa saltare dentro al player tutti i telefoni della band.
  **D3, il conto alla rovescia al rientro** — già registrata come domanda aperta `CD-Q19` in LIBRO Sezione 4. Da sciogliere prima del codice, in sede di contratto CD.
- **Perimetro:** dentro — dove vive l'ascolto del comando del Direttore · chi possiede la memoria dello show · la strumentazione, nello stesso commit della riparazione. Fuori — ⛔ Layer 1, il motore audio, non si tocca · ⛔ la conferma d'uscita dalla stanza è un altro lavoro (non-atomo n.4, Sezione C) · ⛔ il freeze grafico è un altro lavoro.
- **🔴 VINCOLO TECNICO:** le schermate devono osservare **direttamente** il runner o lo stato del motore, **mai attraverso il contenitore della stanza**. Leggere attraverso il contenitore compila, sembra giusto, e non si aggiorna mai. È inciso in BOX3 e ripetuto dentro il codice della stanza. Ignorarlo fa fallire il collaudo su device con l'apparenza di un guasto del motore.
- **CANCELLO:** ratifica tecnica del referee (questa scheda) · OK di Mauro sul diff, cancello distinto · CI · collaudo su device con **DUE apparecchi**, Direttore e Follower. ⚠️ I due apparecchi non sono un lusso: nessuno ha mai riprodotto questa scena con due telefoni, finora è stata solo letta nel codice.
  **Prova aggiuntiva che scioglie D1** — un apparecchio solo, due minuti, tre giri identici. Segnale osservabile, senza ambiguità: alla riapertura dello show il tasto in fondo al dettaglio dice **BACK TO SHOW** (lo show è sopravvissuto) oppure **START SHOW** (lo show è morto).
  ```
  PROVA MEMORIA SHOW — uscita dalla stanza
  giro 1: tasto diceva ______________
  giro 2: tasto diceva ______________
  giro 3: tasto diceva ______________
  il click si era fermato?  sì / no
  altro che ho notato: ______________
  ```
  Giro: carica una setlist dal DEBUG → fai partire lo show → esci dal player col «<» → tocca Casa → rientra in Q-Live → apri lo stesso show → leggi il tasto. ⛔ La setlist si carica dal DEBUG: l'app non sa creare show da zero (`ShowsListView.swift:221` lo dichiara).
- **OPEN:** la gamba mancante del punto (a): il recupero di memoria di iOS. Nessuna prova, né in codice né su device. Non blocca la scheda, resta un buco vero.
- **OPEN:** il disegno del rientro nel player non esiste. Il contratto CD del 18/07 esclude il player per testo proprio. È superficie nuova per CD — è il punto (e), e viene dopo le risposte a (d).
...
⚠️ **MARCATURA 01/09/2026 (A306) — IL PERIMETRO DI QUESTA SCHEDA SI DIMEZZA: SUL DETTAGLIO SENZA OGGETTO, SULLA LISTA VIGENTE. Zero righe sotto riscritte: si marca solo qui.** Il foglio CD del 30/08 (`DESIGN/QLive_Nav/2026-08-30_QLive-Player_IL-VELO-DICE-DA-DOVE...`), riga D④, verbatim: «Abolire la porta è più forte che chiedere conferma: il contratto 18/07 lì non è violato, è rimasto senza oggetto — e resta vigente sulla LISTA, dove le sue due porte esistono ancora.» ⇒ **Del PERIMETRO POSITIVO qui sotto**: il punto **2** (selettore gattato del Dettaglio) è **senza oggetto** — non c'è più selettore da gattare. I punti **1** (le due porte della Lista), **3** (il modale di conferma) e **4** (lettura Start/Stop Sync) **restano vivi**. **Il punto 3 si restringe: il modale era dichiarato «UNICO per i due sfondi», e uno dei due — il Dettaglio — è ora senza oggetto ⇒ resta la sola Lista Shows.** Misurato dal referee il 01/09 contro il PERIMETRO POSITIVO di questa stessa sezione e la riga D④ del foglio CD del 30/08. ⚠️ **Non è una decisione di Mauro: è la registrazione di una dichiarazione di CD del 30/08 che non era incisa in nessun canonico. OK di Mauro al commit, 01/09.**
⚠️ **MARCATURA 03/09/2026 — IL PERIMETRO POSITIVO DI QUESTA SCHEDA È ORA SENZA OGGETTO IN TUTTI E QUATTRO I PUNTI. Zero righe sotto riscritte: si marca solo qui.** Il punto **2** era già senza oggetto dal 01/09. Oggi cadono anche i punti **1**, **3** e **4**, per la ratifica di Mauro del 03/09 — «se lo show è attivo la casa non esiste, l'uscita non esiste» — che **riafferma** la Cond D1 della scheda ⟦S-EXIT⟧ di questo stesso file: «dallo show si esce solo con END SHOW. Nessuna navigazione lo chiude». ⇒ **Se la porta non esiste, non c'è nulla da confermare:** cade il modale (punto 3) e con esso cade **la lettura Layer 2 di Start/Stop Sync** (punto 4) e la questione del chip «N on Link». ⛔ **Cadono le CINQUE PROVE DEVICE del bullet «Gate»**, che collaudavano un overlay che non si costruisce. ⚠️ **La riga ambra NON è ritirata**: trasloca su END SHOW con due inneschi, come già prescrive `BOX5_QBEATS.md` — resta materia CD e resta un OPEN. ✅ **RESTA VIVO di questa scheda il difetto di forma:** il default silenzioso `onSwitch = {}` di `RoomSwitchBar.swift` va tolto comunque, perché una porta non collegata deve diventare errore di compilazione. ⇒ **Il lavoro non-atomo n.4 si trasforma: da «gattare le porte» a «togliere le porte a show attivo».** ⚠️ Marcatura del referee su ratifica di Mauro; OK di Mauro al commit.
