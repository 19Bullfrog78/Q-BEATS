...
private struct SlotBar: View {
    ...
    var body: some View {
        ...
        let bg: Color = active
            ? (stato == "accent" ? .white : stato == "subdiv" ? Color.white.opacity(0.55) : Color.white.opacity(0.92))
        ...
        return Rectangle()
            ...
            .cornerRadius(3)
            .opacity(opacity)
            .shadow(color: active && stato == "accent" ? Color.white.opacity(0.6) : .clear, radius: 4)
    }
}
