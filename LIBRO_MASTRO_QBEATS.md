...
**Versione:** 15 (proposta, in attesa ratifica esplicita Mauro)
**Ultima modifica:** 2026-06-11 (Bug 2.b Ferita A+B CHIUSO e mergiato su master `ee0cbc0` via squash — chiude il fronte cross-device sync sezione/time-sig, era bloccante palco; nuova domanda di design END SHOW = CD-Q5 → CD)
**Edit author:** CC chat principale 29/05/2026
...
| Fase 6-7-bis | Sync sezione runtime cross-device + counter offset `bar 2 di N` + Issue A/B | aperto 2026-05-29 | 2026-05-29 | Raccoglie i punti rimandati da Fase 6-7: **Bug 2.b** (counter Follower `bar 2 di N` + desync visivo battute/sezione/time-sig cross-device — `BUGS_QBEATS.md` v2); sync sezione runtime Director→Follower; **Issue A** (titolo header troncato iPhone, dominio CD); **Issue B** (microbar fantasma pre-Play); header BPM pre-Play primo caricamento; **CANCEL → Select Setlist** (rework nav LiveRootView, vedi caveat sez. 2 29/05) |
...
- **Repo Q-BEATS**: `C:\Users\BULLFROG\Desktop\ANTIGRAVITY\Q-BEATS\` — GitHub: `https://github.com/19Bullfrog78/Q-BEATS` branch `master`. HEAD corrente: `ee0cbc0` (Bug 2.b Ferita A+B squash 11/06; `BUGS_QBEATS.md` v6). Storico: `ff530b7` (Fase 6-7) → `bbdd06a` (libro mastro v15, parent diretto dello squash `commit-tree -p bbdd06a`) → `ee0cbc0`.
...
- **Tracker bug centralizzato**: `BUGS_QBEATS.md` in root repo, **v2** (commit `ff530b7`, 29/05/2026) — chiusure Fase 6-7 (Problema B, Bug 5, Bug 5-BPM, 1.j) + nuove voci aperte (Bug 2.b, header BPM pre-Play, Issue A/B). Creazione v1 in `153711c`, 27/05/2026 mattina.
