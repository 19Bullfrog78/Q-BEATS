BOX3 V97 — 2026-07-18 (AUTOPORTANTE) · HEAD=origin=f8276f6 (⚠️ campo strutturalmente stale by-design, come Sez.5 di LIBRO: cita l'HEAD al momento della scrittura, NON aggiornato dopo; chi ha bisogno dell'HEAD vero verifica `git log -1` a fonte — verificato 18/07: HEAD reale = `f8276f6` = S4a «QLiveShowsView come dead code», CI `29653954692` verde)
...
Mirror di consegna standing di QUESTO BOX3 V95 (2 target, come V94 — §7 punto 4 sopra):
`ARCHIVIO.MD\14_07_2026\` nel repo (gitignored, sotto
`C:\Users\BULLFROG\Desktop\ANTIGRAVITY\Q-BEATS\`) + `E:\HOBBY\MUSICA - BATTERIA -SISTA\Q-BEATS\
FILE X CLAUDE.MD\BOX3_Codice\`. In PIÙ, se si assembla un bundle di sync col referee per il
14/07, una copia va anche in `REFEREE_SYNC_2026-07-14\` (trasporto ad-hoc, NON un terzo
mirror standing — §7 punto 4). Nessun commit git (BOX3 è untracked). Read-back sha256 dei
target da dichiarare nel messaggio di consegna DOPO ratifica, NON inciso qui (R7).

**STATO DI QUESTO FILE:** V95 canonica corrente — diff ratificato dal referee, gli ultimi
residui corretti su suo rilievo (le 2 sintesi di §14 che ripetevano la vecchia formula errata
sulle stime BOX5, e questo stesso blocco di coda che prima si auto-descriveva come bozza non
propagata), quindi propagato ai 2 mirror standing sopra. Doc-commit LIBRO associato: git `872dd5b`. Il read-back sha256 dei mirror è dichiarato nel MESSAGGIO di
consegna, NON inciso qui — un documento non contiene il proprio hash né una descrizione di sé
che diventa falsa appena il file è in posizione (vedi regola ULTIMO-BLOCCO, §12).
